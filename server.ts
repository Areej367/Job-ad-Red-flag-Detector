...
// Use DEFAULT_APP_PORT (3000) inside AI Studio sandbox (behind nginx),
// or process.env.PORT in deployed Cloud Run container, defaulting to 3000.
...
// API routes FIRST
app.post(["/api/analyze-job", "/api/analyze-job/"], async (req, res) => {
  try {
    const { jobText, image } = req.body;
...
8. Poor Professional Quality: Excessive emojis (🔥💰💵), erratic capitalization, broken English or Urdu romanization, generic copy-pasted templates.
...
- verdict: A crisp 1-2 sentence plain-language verdict summary.`;
    ...
    const errorMessage =
      error?.message?.includes("GEMINI_API_KEY")
        ? "Gemini API key is not configured. Please check your environment configuration."
        : "Failed to analyze the job posting. The AI service may be temporarily busy. Please retry in a moment.";
